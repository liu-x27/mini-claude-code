...
`wc -l src/agent.ts` scored 0.066 and ran — the green pill is the only trace, because
...
`rm -rf dist` scored 0.993 and stopped. The card names the command rather than the tool,
since "Bash" is not a decision anyone can make and `rm -rf dist` is, and it shows what
deferred it: a call held at 0.21 deserves a different glance from one held at 0.993.
...
The client has two themes over the same components, switched from the top bar: the
light one above, and a terminal one, where the same approval reads as a prompt in a
log:

![The same approval in the terminal theme](docs/terminal-theme.png)

All three images come from one real run against a real judge — the loop and the judge
